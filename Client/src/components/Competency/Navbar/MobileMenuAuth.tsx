...
const MobileMenuAuth: React.FC<MobileMenuAuthProps> = ({ menuOpen, isLoggedIn, navItemsCount, onLogin, onLogout, onClose }) => {
  return (
    <div
      className={`flex flex-col pt-3 mt-6 mx-5 border-t border-gray-200 transform transition-all duration-300 ${menuOpen ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}
      style={{
        transitionDelay: menuOpen ? `${200 + navItemsCount * 100 + 100}ms` : "0ms",
      }}
    >
      {isLoggedIn ? (
        <button
          ...
          style={{
            transitionDelay: menuOpen ? `${200 + navItemsCount * 100 + 150}ms` : "0ms",
          }}
        >
          <span>Sign Out</span>
        </button>
